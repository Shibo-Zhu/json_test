...
class PLCControler(object):
    ...
    def AddConfNodeTypesList(self, typeslist):
        ...
        addedcat = [{"name": ("%s POUs") % confnodetypes["name"],
    ...
    def GetConfNodeDataTypes(self, exclude=None, only_locatables=False):
        return [{"name": ("%s Data Types") % confnodetypes["name"],
                 "list": [
                     datatype.getname()
    ...
    def fbdSetConnectionWires(self, connection, connector):
        wires = connector.GetWires()
        idx = 0
        for wire in wires:
            points = wire.GetPoints()
            result = wire.GetConnectedInfos()
            if result is not None:
            ...
            for param, value in infos.items():
                if param == "name":
                    ...
                    for connector in value["inputs"]:
                        ...
                        self.fbdSetConnectionWires(variable.connectionPointIn, connector)
            ...
            for param, value in infos.items():
                if param == "name":
                    ...
                    if len(value["inputs"]) > 0:
                        ...
                        self.fbdSetConnectionWires(variable.connectionPointIn, input)
            ...
            for param, value in infos.items():
                if param == "name":
                    ...
                    if isinstance(connection, PLCOpenParser.GetElementClass("continuation", "commonObjects")):
                        ...
                        self.fbdSetConnectionWires(connection.connectionPointIn, value)
                ...
                if param == "height":
                    ...
                    if isinstance(powerrail, PLCOpenParser.GetElementClass("leftPowerRail", "ldObjects")):
                        ...
                        for connector in value["inputs"]:
                            ...
                            connection.setrelPositionXY(position.x, position.y)
                            self.fbdSetConnectionWires(connection, connector)
        ...
        if element is not None:
            ...
            for param, value in infos.items():
                if param == "name":
                    ...
                    self.fbdSetConnectionWires(contact.connectionPointIn, input_connector)
        ...
        if element is not None:
            ...
            for param, value in infos.items():
                if param == "name":
                    ...
                    self.fbdSetConnectionWires(coil.connectionPointIn, input_connector)
                    output_connector = value["outputs"][0]
                    position = output_connector.GetRelPosition()
            ...
            for param, value in infos.items():
                if param == "name":
                    ...
                    if len(value["inputs"]) > 0:
                        ...
                        self.fbdSetConnectionWires(step.connectionPointIn, input_connector)
                    else:
        ...
        if element is not None:
            ...
            for param, value in infos.items():
                if param == "type" and value != "connection":
                    ...
                    self.fbdSetConnectionWires(transition.connectionPointIn, input_connector)
                    output_connector = value["outputs"][0]
                    position = output_connector.GetRelPosition()
                    ...
                    self.fbdSetConnectionWires(transition.condition.content, value)
            ...
            for param, value in infos.items():
                if param == "height":
                    ...
                    if divergence_type in ["selectionDivergence", "simultaneousDivergence"]:
                        ...
                        self.fbdSetConnectionWires(divergence.connectionPointIn, input_connectors[0])
                    else:
                        ...
                        for input_connector in input_connectors:
                            ...
                            connection.setrelPositionXY(position.x, position.y)
                            self.fbdSetConnectionWires(connection, input_connector)
        ...
        if element is not None:
            ...
            for param, value in infos.items():
                if param == "target":
                    ...
                    self.fbdSetConnectionWires(jump.connectionPointIn, value)
        ...
        if element is not None:
            ...
            for param, value in infos.items():
                if param == "actions":
                    ...
                    self.fbdSetConnectionWires(actionBlock.connectionPointIn, value)
