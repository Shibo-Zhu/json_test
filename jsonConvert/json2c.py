...
with open(os.path.join(filedir, 'OT_SFC.json'), 'w') as f:
    json.dump(OT_data, f, indent=4)
...
BuildPath = os.path.join(current_directory, "build")
if not os.path.exists(BuildPath):
    os.makedirs(BuildPath)
iec2c_libpath = os.path.join(current_directory, "matiec_windows","lib")
Cmd = os.path.join(current_directory, "matiec_windows", "iec2c.exe")
...
logging.basicConfig(level=logging.INFO,
                    ...
                    handlers=[logging.FileHandler(os.path.join(BuildPath, 'build.log')),
                              logging.StreamHandler()])
logger = logging.getLogger()
# logger.warning

status, result, err_result = ProcessLogger(logger, buildcmd,
                                                       no_stdout=True,
                                                       no_stderr=True).spin()