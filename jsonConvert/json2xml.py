...
# send ST code
try:
    files = {'file': open(program_filepath, 'rb')}
    url_file = 'http://' + URL + ':5001' + ST_UPLOAD_ROUTE
    response = requests.post(url_file, files=files)
    print('Status Code:', response.status_code)
    print('Response Body:', response.text)
    log_path = os.path.join(ProjectPath, 'log.txt')
    ...
    print('Error in sending ST code')
    print(traceback.format_exc())
    

# send IT code
try:
    for IT_file in code_index:
        IT_file_path = os.path.join(ProjectPath, IT_file)
        files = {'file': open(IT_file_path, 'rb')}
        url_file = 'http://' + URL +  ':5001' + IT_UPLOAD_ROUTE
        response = requests.post(url_file, files=files)
        print('Status Code:', response.status_code)
        print('Response Body:', response.text)
    log_path = os.path.join(ProjectPath, 'log.txt')
